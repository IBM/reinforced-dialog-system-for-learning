...
class GPTQA:
    def __init__(self):
        ...
        if os.path.exists(self.args.model_file_path):
            print('Loading exisitng model at ' + str(self.args.model_file_path))
            sys.stdout.flush()
            self.generator = AutoModelForCausalLM.from_pretrained(self.args.model_name,
                                                                    state_dict=torch.load(self.args.model_file_path)['model'])
        else:
            print('Loading default pre-trained GPT weights')
            assert 'gpt' in self.args.model_name or 'GPT' in self.args.model_name
            sys.stdout.flush()
            self.generator = AutoModelForCausalLM.from_pretrained(self.args.model_name)
        self.generator.to(self.device)
    ...
    def convert_examples_to_features(self, examples):
        ...
        for e in tqdm(examples, desc='Examples'):
            ...
            if self.args.experiment_type == 'chat_only':
                ...
                context = e.context
                source ='document: ' + context + self.tokenizer.eos_token + ' chat: ' + chat_history + self.tokenizer.eos_token
            elif self.args.experiment_type == 'chat_wizard':
                context = e.context
                context = ast.literal_eval(context)
                all_docs = ''
                for doc in context:
                    ...
                    passage = ' '.join(doc[title])
                    all_docs = all_docs + ' title: ' + title + ' text: ' + passage
                source = 'document: ' + all_docs + self.tokenizer.eos_token + ' chat: ' + chat_history + self.tokenizer.eos_token
            else:
                print('Unrecongnized argument for experiment type')
            ### No SEP token between context and chat_history
            source_tokens = self.tokenizer.tokenize(source)[:self.args.source_max_len - 2]
            ...
            target_len = len(target_ids)
            padding_len = self.args.target_max_len - target_len
            target_ids += ([0] * padding_len)
            target_labels += ([-100] * padding_len)  # -100 is the default index to be ignored
            assert len(target_ids) == self.args.target_max_len
            assert len(target_labels) == self.args.target_max_len
            f = InputFeatures(index, source_ids, source_mask, source_len, target_ids, target_labels, target_len)
            features.append(f)
            index += 1
        return features
    ...
    def train(self):
        ...
        cached_features_devfile = os.path.join(
            self.args.data_dir,
            "cached_GPT_{}_task_{}_dev_srcLen{}_tgtLen{}".format(
                self.tokenizer.__class__.__name__,
                self.args.experiment_type,
                str(self.args.source_max_len),
                str(self.args.target_max_len),
        ...
        cached_features_trainfile = os.path.join(
            self.args.data_dir,
            "cached_GPT_{}_task_{}_train_srcLen{}_tgtLen{}".format(
                self.tokenizer.__class__.__name__,
                self.args.experiment_type,
                str(self.args.source_max_len),
                str(self.args.target_max_len),
